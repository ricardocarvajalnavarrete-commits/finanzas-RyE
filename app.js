...
async function compModal(id){
 const d=debtById(id);if(!d)return;
 const enNube=!!(fb.user&&fb.loaded&&d.compPath);
 const blob=enNube?null:await docGet(id+'_comp');
 const tiene=enNube||!!blob;
 openModal('🧾 Comprobante de pago: '+esc(d.nombre),`<p class="mut">${enNube?'☁️ En la nube (todos los equipos)':blob?'📱 Solo en este dispositivo':'⚪ Sin comprobante adjunto'}</p>
 ...
 $('#comp-ver').onclick=async()=>{try{if(enNube){openViewer(d,await compUrl(d.compPath),compModal);}else{const b=await docGet(id+'_comp');if(!b)return toast('❌ Sin comprobante');openViewer(d,URL.createObjectURL(b),compModal);}}catch(e){toast('❌ No se pudo abrir');}};
 $('#comp-sub').onclick=()=>{const i=document.createElement('input');i.type='file';i.accept='application/pdf,.pdf';i.onchange=async()=>{const f=i.files[0];if(!f)return;if(f.type!=='application/pdf'&&!f.name.toLowerCase().endsWith('.pdf'))return toast('⚠️ Solo PDF');await docPut(id+'_comp',f);d.compPdf=true;save();if(fb.user&&fb.loaded){try{d.compPath=await compUpload(id,f);save();toast('☁️ Comprobante subido a la nube');}catch(e){d.compPath=null;toast('📱 Comprobante en este equipo');}}else toast('📱 Comprobante en este equipo');compModal(id);};i.click();};
 $('#comp-del').onclick=async()=>{if(d.compPath)await compDelete(d.compPath);await docDel(id+'_comp');d.compPath=null;d.compPdf=false;save();toast('🗑️ Comprobante eliminado');compModal(id);};
}
/* ============================== VISTAS ============================== */
...
function openPagoModal(id){const d=debtById(id);if(!d)return;const min=d.sinVencimiento?cicloRestante(d):minPago(d);
 ...
 closeModal();
 if(f){if(f.type!=='application/pdf'&&!f.name.toLowerCase().endsWith('.pdf'))toast('⚠️ El comprobante debe ser PDF');else{const dd=debtById(id);if(dd){docPut(id+'_comp',f);dd.compPdf=true;if(fb.user&&fb.loaded){compUpload(id,f).then(p=>{dd.compPath=p;save();toast('☁️ Comprobante subido a la nube');}).catch(()=>toast('📱 Comprobante en este equipo'));}else toast('📱 Comprobante en este equipo');}}}
 confirmarPago(id,fecha,monto,nota);};}
...
function tarjetaModal(id){const t=id?db.tarjetas.find(x=>x.id===id):{persona:db.personas[0],formato:'Física',tipo:'Tarjeta Débito'};
 const optsVinc=[['','-- Sin vincular --'],...db.cuentas.filter(c=>!c.archivada).map(c=>[c.id,c.persona+' · '+c.banco+' · '+c.tipo+' · '+mask(c.numero)])];
...
function renderCuentas(){
 ...
 const vincInfo=t=>{const vc=db.cuentas.find(c=>c.id===t.vinculadaCuentaId);return vc?`${esc(vc.tipo)} ${mask(vc.numero)} · ${esc(vc.banco)}`:'—';};
 ...
 <div class="card"><h3>Cuentas (${cs.length})</h3><div class="tblwrap"><table><tr><th>Titular</th><th>Banco</th><th>Número de cuenta</th><th></th></tr>${cs.map(c=>`<tr><td>${esc(c.persona)}</td><td>${esc(c.banco)}</td><td class="mask">${showNums?esc(c.numero):mask(c.numero)}</td><td><button class="btn mini" data-act="edit-cuenta" data-id="${c.id}">✏️</button><button class="btn mini" data-act="arch-cuenta" data-id="${c.id}">📦</button></td></tr>`).join('')||'<tr><td colspan="4" class="mut">Sin resultados.</td></tr>'}</table></div></div>
 ...
 <div class="card"><h3>Tarjetas (${ts.length})</h3><div class="tblwrap"><table><tr><th>Titular</th><th>Entidad</th><th>Tipo</th><th>Formato</th><th>Número</th><th>Vinculada a</th><th></th></tr>${ts.map(t=>`<tr><td>${esc(t.persona)}</td><td>${esc(t.entidad)}</td><td>${esc(t.tipo)}</td><td>${t.formato==='Virtual'?'🌐 Virtual':t.formato==='Ambas'?'💳🌐 Ambas':' Física'}</td><td class="mask">${showNums?esc(t.numero):mask(t.numero)}</td><td>${vincInfo(t)}</td><td><button class="btn mini" data-act="edit-tarjeta" data-id="${t.id}">✏️</button><button class="btn mini" data-act="arch-tarjeta" data-id="${t.id}">📦</button></td></tr>`).join('')||'<tr><td colspan="7" class="mut">Sin resultados.</td></tr>'}</table></div></div>`;
...
document.addEventListener('click',e=>{const b=e.target.closest('[data-act]');if(!b)return;const {act,id}=b.dataset;const find=(arr,i)=>arr.find(x=>x.id===i);
 ...
 switch(act){
  ...
  case 'comp-deuda':compModal(id);break;
  ...
  case 'exp-boveda-pdf':exportBovedaPDF();break;case 'imp-excel':importarExcel();break;case 'exp-excel':exportarExcel();break;
