...
function selBanco(id,label,val){const opts=db.acreedores.filter(a=>a.tipo==='financiera'||a.tipo==='empresa').map(a=>[a.nombre,a.nombre]);if(val&&!opts.some(o=>o[0]===val))opts.unshift([val,val]);opts.push(['__new','➕ Añadir nueva…']);return sel(id,label,opts,val);}
